  ...
  const getUserData = async () => {
    if (typeof window.ethereum !== "undefined") {
      ...
      try {
        ...
        setAllocatedBudget(ethers.formatEther(allocatedBudgetResponse));

        const requestedFundsResponse = await contract.getRequestFunds(userAddress);
        setRequestedFunds(ethers.formatEther(requestedFundsResponse));

        const spentFundsResponse = await contract.getSpentFunds(userAddress);
        setSpentFunds(ethers.formatEther(spentFundsResponse));

        const fetchBudget = await contract.getTotalBudget();
        setTotalBudget(ethers.formatEther(fetchBudget));
      } catch (error) {
  ...
  const requestFunds = async () => {
    if (typeof window.ethereum !== "undefined") {
      ...
      try {
        ...
        toast.success("Funds successfully requested:", receipt);
      } catch (error) {
        toast.error("Error requesting funds:", error.message);
      }
    }
  };
